    ...
    def reset_stateful_objects(self, side='both'):
        ...
        if self.left:
            ...
            if ':' not in left_creds['service']:
                self.left_adapter = SumoUserAdapter(left_creds, 'left', log_level=self.mainwindow.log_level)

        if self.right:
            right_creds = self.mainwindow.get_current_creds('right')
            if ':' not in right_creds['service']:
                self.right_adapter = SumoUserAdapter(right_creds, 'right', log_level=self.mainwindow.log_level)
